...
|------|--------|-----------------|
| 1 | In a logged-out session, navigate to `/` | Redirected to `/login?next=/` or login page. Dashboard is not visible. | ✅ pass |
| 2 | Logged out, navigate to `/profile/` | Redirected to login page. | ✅ pass |
| 3 | Logged out, navigate to `/start/`, `/quick-test/start/`, or `/assistant/` | Redirected to login page. | ✅ pass |
| 4 | Log in as regular user | Redirected to dashboard. | ✅ pass |
| 5 | As regular user, navigate to `/configuration/` | Access is denied or redirected. Configuration secrets page is not visible. | ✅ pass |
| 6 | As regular user, navigate to `/targets/`, `/executors/`, `/history/`, and `/assistant/` | Pages load successfully. | ✅ pass |
| 7 | Log out regular user | Session is cleared and login page is shown. | ✅ pass |
| 8 | Log in as Admin-group user | Redirected to dashboard. | ✅ pass |
| 9 | As Admin user, navigate to `/configuration/` | Configuration page loads successfully. | ✅ pass |
| 10 | Navigate to `/settings/` as Admin user | Permanently redirects to `/configuration/`. | ✅ pass |
...
- [X] Access denied or redirect behavior is clear to the user
...
- [X] Logout invalidates access to protected pages
...
## Defects Found
 ...
 2 defects found
    ...
    1.Logout doesn't invalidates access to protected pages
    2.Access denied or redirect behavior is not clear to the user
